...
class Engine:
    ...
    def minimax(self, board: Board, ai_turn: bool, depth: int, alpha: float,
        ...
        if ai_turn:
            #the ai will choose the outcome where the opponent wins, so instead of maximizing its outcome
            #it will minimize its outcome
            min_eval = float('inf')
            best_move = None
            for move in available_moves:
                ...
                board.undo(move)
                #the ai will prefer the lowest score
            ...
            #similar to the previous if statement but the opponent move will be maximized
            max_eval = float('-inf')
            best_move = None
            for move in available_moves:
